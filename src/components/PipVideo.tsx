...
const PipVideo = ({ onExpand, onClose }: PipVideoProps) => {
  ...
  // Auto-advance to next video (simple: on each key change the iframe replays)
  const currentVideo = bondVideos[currentIndex];

  if (!visible) return null;

  const embedUrl = `https://www.youtube.com/embed/${currentVideo.id}?autoplay=1&mute=${muted ? 1 : 0}&loop=1&playlist=${bondVideos.map(v => v.id).join(",")}&controls=0&rel=0&modestbranding=1&playsinline=1`;
